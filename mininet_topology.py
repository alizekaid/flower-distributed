...
class FlowerTopology:
    ...
    def create_topology(self):
        ...
        # ── Switches (K4: every pair connected) ──────────────────────────────
        # s1 (dpid=1): c1, c2
        # s2 (dpid=2): c3
        # s3 (dpid=3): c4
        # s4 (dpid=4): h1  ← server, most central
        ...
        self.switch = switches['s1']
        
        # ── Inter-switch links (fully-connected mesh) ─────────────────────────
        info("*** Creating switch-to-switch links (K4 mesh)...\n")
        sw_links = [
            ('s1', 's2'),
            ('s1', 's3'),
            ('s1', 's4'),
        ...
        self.clients = [c1, c2, c3, c4]

        # ── Host ↔ Switch links ───────────────────────────────────────────────
        info("*** Connecting hosts to switches...\n")
        # Server on s4 (most connected switch → fewest hops to all clients)
        self.net.addLink(self.server, switches['s4'], bw=config.SERVER_BW, delay=config.DELAY)
        # c1, c2 → s1
        ...
        # c3 → s2
        self.net.addLink(c3, switches['s2'], bw=config.CLIENT_BW, delay=config.DELAY)
        # c4 → s3
        self.net.addLink(c4, switches['s3'], bw=config.CLIENT_BW, delay=config.DELAY)
    ...
    def _print_network_info(self):
        ...
        info("\n" + "="*60 + "\n")
        info("Network Topology (4-switch K4 diamond):\n")
        info("="*60 + "\n")
        info("  c1, c2  ──  s1  ──────────────  s4  ── h1 (server)\n")
        info("                \\   \\          /   /\n")
        info("                 s3  ──────────  s2\n")
        info("                  |               |\n")
        info("                  c4              c3\n")
