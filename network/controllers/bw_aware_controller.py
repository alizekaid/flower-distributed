...
import sys
import os
# Add project root to path for ryu-manager imports
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))
from network.managers.network_managers import NetworkManager
from network.managers.stats_manager import get_stats_manager
import time
import copy
...
class BWAwareController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.topology_file = "network/topology/topology.json"
        self.stats = {} # (dpid, port) -> (rx_bytes, tx_bytes, timestamp)
        self.active_paths = {} # (src, dst) -> path object
        
        # Logging setup
        import logging
        fh = logging.FileHandler('logs/bw_controller.log', mode='w')
    ...
    def _packet_in_handler(self, ev):
        ...
        if sw_obj:
            ...
            if not host_obj or host_obj.switch_id != sw_obj.name or host_obj.switch_port != in_port:
                from network.managers.network_managers import Host
