...
class StatsManager:
    ...
    """
    def __init__(self, topology_file="network/topology/topology.json", usage_file="usage_stats.json"):
        # Resolve topology file relative to project root if not absolute
        if not os.path.isabs(topology_file):
            base_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.abspath(os.path.join(base_dir, "..", ".."))
            self.topology_file = os.path.join(project_root, topology_file)
        else:
            self.topology_file = topology_file
            
        self.usage_file = os.path.abspath(usage_file)
        self.links = {}
        self.load_stats()
        self.load_usage()
    ...
    def load_stats(self):
        ...
        if not os.path.exists(self.topology_file):
            print(f"ERROR: Topology file not found at {self.topology_file}")
            return False
...
def get_stats_manager(topology_file="network/topology/topology.json"):
    global _provider
    if _provider is None:
        # Resolve usage file relative to project root
        base_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(base_dir, "..", ".."))
        usage_file = os.path.join(project_root, "logs", "usage_stats.json")
